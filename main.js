...
class WebSocketClient {
    ...
    connect() {
        ...
        this.socket.onopen = () => {
            log.info("WebSocket连接已建立，节点:", this.uuid);
            this.reply(this.regNode);
        };

        this.socket.onmessage = (event) => {
            if (event.data === "2" || event.data === "41") this.socket.send("3");
            else log.info(`节点 ${this.uuid} 收到消息:`, event.data);
        };

        this.socket.onclose = () => {
            log.warn("WebSocket连接已关闭，节点:", this.uuid);
            if (this.shouldReconnect) {
                log.warn(`${this.reconnectInterval / 1000} 秒后重新连接节点:`, this.uuid);
                setTimeout(() => this.connect(), this.reconnectInterval);
            }
        };

        this.socket.onerror = (error) => {
            log.error(`节点 ${this.uuid} WebSocket错误:`, error.message);
            this.socket.close();
        };
    }
...
async function registerNode(token, proxy = null, node = null) {
    ...
    while (retries < maxRetries) {
        try {
            ...
            log.info("节点注册成功:", response.data);
            return uuid;
        } catch (error) {
            log.error("注册节点时出错:", error.message);
            retries++;
            if (retries < maxRetries) {
                log.info("10秒后重试...");
                await new Promise(resolve => setTimeout(resolve, 10000));
            } else {
                log.error("已达到最大重试次数，放弃注册。");
                return null;
            }
        }
    }
...
async function confirmUser(token, proxy = null) {
    ...
    try {
        ...
        log.info("确认用户响应:", response.data);
        return null;
    } catch (error) {
        log.info("确认用户:", error.message);
        return null;
    }
};
    ...
    if (questsIds && questsIds.length > 0) {
        ...
        for (const questId of questsIds) {
            ...
            try {
                ...
                log.info(`任务 ${questId} 完成成功。`);
            } catch (error) {
                log.error(`完成任务 ${questId} 时出错:`, error);
            }
        }
    } else {
    ...
    try {
        ...
        await Promise.all(tokens.map(async (token, index) => {
            ...
            setInterval(async () => {
                ...
                log.info(`账户 #${index + 1} 的用户信息:`, { Active_Nodes: nodes.length, users });
        ...
        log.info("所有账户连接已建立 - 保持运行即可。");
    } catch (error) {
        log.error("WebSocket连接错误:", error.message);
    }
}
